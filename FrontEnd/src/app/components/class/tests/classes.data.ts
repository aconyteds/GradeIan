...
import { Class, AssignmentItem, Grade } from "../../../interfaces";
...
export class ClassesServiceStub {
  ...
  public classDetails = new ClassDetailModel(0,
  {
  ...
  public getClass(classIdentifier: number): Observable<any> {
    if (classIdentifier > 0) {
      this.classDetails.response = true;
      return of(this.classDetails);
    } else {
      return of({
        response: false
      });
  ...
  public getAssignmentGrades(assignmentItems: AssignmentItem []): Observable<any> {
    return of({grades: this.classDetails.grades});
  }

  public saveGrade(newGrade: Grade): Observable<any> {
    const found = this.classDetails.grades.some((grade) => {
      if (grade.assignmentId === newGrade.assignmentId && grade.studentId === newGrade.studentId) {
        grade = newGrade;
        return true;
      }
    });
    if (!found) {
      this.classDetails.grades.push(newGrade);
    }
    return of({response: true});
  }

  public saveGrades(newGrades: Grade[]): Observable<any> {
    const processes = newGrades.map((grade) => this.saveGrade(grade));
